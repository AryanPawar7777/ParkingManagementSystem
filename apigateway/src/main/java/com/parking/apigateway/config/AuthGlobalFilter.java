...
import com.parking.apigateway.dto.AuthDto; // ✅ correct import path
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        return responseMono.flatMap(response -> {
            if (response.getStatusCode().is2xxSuccessful()) {
                ...
                // ✅ FIXED: method name is buildAuthHeader (not BuildAuthHeader)
                ServerHttpRequest mutatedRequest = exchange.getRequest()
                        .mutate()
                        .header(HttpHeaders.AUTHORIZATION, _AuthFactory.buildAuthHeader(routeId))
                        .header("X-API-GATEWAY-SECRET", _AuthFactory.getSharedSecret()) // ✅ correct
                        .build();
